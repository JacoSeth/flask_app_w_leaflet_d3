...
var congressionalGeoLayer = new L.GeoJSON(stateCongressionalDistricts, {
  ...
  onEachFeature: function(feature, layer) {
    ...
    layer.bindPopup("<hr> <h2>" + "Congressional District " + feature.properties.FID + "</h2> <hr> <h2>" + "Total Subscribers :" + totalSubscribers[totalSubscribers.length-1] + "</h2>")
  }
});
  ...
  for (var i = 0; i < userData.length; i++) {
    ...
    if (
      ...
      userMarkers.push(
          ...
          .bindPopup('<h5>' + 'Role: ' + user.roles + '</h5>' + '<h5>' + 'County: ' + user.county + '</h5>' +'<h5>' + 'Congressional District: ' + user.cd + '</h5>' + '<h5>' + 'State Senate District: ' + user.sd + '</h5>' + '<h5>' + 'State House District: ' + user.hd + '</h5>')
          .addTo(userLayer))
